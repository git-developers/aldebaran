...
class TextareaElement extends gridLayout implements IElement
{
    ...
    function processTag(Form $form, Element $element)
    {
        ...
        $out .= "<span class='input-group-addon' title='{$element->getTagName()}'><i class='fa fa-fw fa-align-justify'></i></span>";
        $out .= "<textarea
                    class='form-control'
                    name='saveEventForm[{$form->getUniqueId()}][{$element->getTagName()}_{$element->getUniqueId()}]'
                    id='{$element->getTagId()}'
                    placeholder='{$element->getTagPlaceholder()}'
                    rows='{$element->getTagRows()}'
                    cols='{$element->getTagCols()}'>{$element->getTagValue()}</textarea>";
